...
function formatProductionStatus(value) {
            ...
            <div className="space-y-2">
              {recentJobs.map((job) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-lg border border-zinc-200 px-3 py-2 text-left transition-colors hover:bg-zinc-100 dark:border-zinc-800 dark:hover:bg-zinc-800"
                >
                  <span>
                    <span className="block text-sm font-medium text-zinc-900 dark:text-zinc-100">{job.customerName || 'Customer: N/A'}</span>
                    <span className="block text-xs text-zinc-500 dark:text-zinc-400">
                      {job.name || 'Untitled Job'} | Status: {formatProductionStatus(job.productionStatus)} | Style: {styleNameById.get(job.defaultStyleId) || 'N/A'} | Overlay:{' '}
                      {overlayNameById.get(job.defaultOverlayCategoryId) || 'N/A'}
                    </span>
                  </span>
                  <span className="text-xs text-zinc-500 dark:text-zinc-400">{formatDate(job.createdDate)}</span>
                </button>
              ))}
            </div>
          ) : null}
